...
fn output_variant(o: &Output<'_>) -> &'static str {
    match o {
        ...
        Output::BufferTooSmall { .. } => "BufferTooSmall",
        _ => "Unknown",
    }
}
        ...
        loop {
            match client.poll_output(&mut big_buf) {
                ...
                Output::Connected => {
                    ...
                    let tiny = client.poll_output(&mut tiny_buf);
                    assert!(
                        matches!(tiny, Output::Timeout(_) | Output::BufferTooSmall { .. }),
                        "undersized buffer must yield Timeout or BufferTooSmall even \
                         with queued packets behind the CID event, got {}",
                        output_variant(&tiny)
                    );
...
fn dtls12_cid_client_rejects_unsolicited_server_hello_extension() {
    ...
    match result {
        Err(dimpl::Error::SecurityError(err)) => {
            assert_eq!(
                err,
                dimpl::SecurityError::UnsolicitedConnectionIdExtension,
...
fn dtls12_cid_server_rejects_malformed_extension() {
    ...
    match result {
        Err(dimpl::Error::SecurityError(err)) => {
            assert_eq!(
                err,
                dimpl::SecurityError::MalformedConnectionIdExtension,
...
fn dtls12_cid_with_no_dtls12_suites_fails_config_build() {
    ...
    match result {
        Err(Error::ConfigError(err)) => {
            assert_eq!(
                err,
                dimpl::ConfigError::ConnectionIdWithoutDtls12CipherSuite,
                "ConfigError should explain the CID/DTLS 1.2 mismatch: {}",
                err
            );
        }
        other => panic!(

...
fn dtls13_client_rejects_unsolicited_connection_id_in_server_hello() {
    ...
    match result {
        ...
        Ok(()) => {
            // Transient parse failures no longer surface: a datagram the
            // record parser cannot frame is silently discarded and
            // `handle_packet` returns `Ok`. The spliced ServerHello was
            // dropped wholesale, so the forbidden extension was never
            // accepted — the handshake cannot advance on this packet,
            // which still satisfies RFC 8446 §4.2.
            // The client made no progress: the next poll only re-emits
            // flight-1 retransmits, never a Connected event.
        }
...
fn dtls13_new_13_with_cid_config_still_rejects_unsolicited_sh_extension() {
    ...
    match result {
        ...
        Ok(()) => {
            // Transient parse failures are silently discarded (datagram
            // dropped wholesale, handshake cannot advance) — the
            // unsolicited `0x0036` was never accepted, satisfying
            // RFC 8446 §4.2.
        }
        other => panic!("unexpected error: {:?}", other),
    }

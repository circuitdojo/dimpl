...
impl Record {
    ...
    fn decrypt_record(
        ...
        let (_, dtls) = DTLSRecord::parse(&buffer, 0, 0)
            .map_err(|_| InternalError::parse(nom::error::ErrorKind::Fail))?;

        let explicit_nonce_len = decrypt.explicit_nonce_len();
        let aead_overhead = decrypt.min_protected_fragment_len();
        ...
        let new_len = {
            ...
            // once application data flows, preserving the association. During
            // the handshake a decrypt failure stays fatal so active tampering
            // is not masked. The replay window is intentionally NOT advanced
            // on failure (RFC 6347 §4.1.2.6), so a legitimate retransmit at
            // the same sequence still decrypts.
        ...
        // For CID records, report the recovered inner content type — the
        // outer type is always tls12_cid.
        let real_content_type = match cid_inner_rewrite {
            Some((content_type_byte, _)) => ContentType::from_u8(content_type_byte),
            None => dtls.content_type,
        };
        decrypt.note_decrypted_record(real_content_type);
...
pub trait RecordHandler {
    ...
    ///   negotiated inbound length was zero (legacy RFC 6347 framing).
    /// - `Some(cid)` — CID was negotiated with non-zero inbound length;
    ///   incoming records use `tls12_cid` framing with `cid.len()` bytes
    ///   between the sequence-number and length fields.
    fn our_cid(&self) -> Option<&[u8]>;
    /// Returns `Some(expected_cid_bytes)` once inbound CID is armed — i.e.
    /// the peer's ChangeCipherSpec has been processed; `None` otherwise.
